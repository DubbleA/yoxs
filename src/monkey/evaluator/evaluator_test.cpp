...
void TestEvalIntegerExpression(){
    ...
    for(const auto& tt : tests){
        ...
        printf("Test '%s' evaluated to address: %p\n", tt.input.c_str(), (void*)evaluted.get());
...
std::shared_ptr<Object> testEval(const std::string& input) {
    ...
    std::cout << "Program AST: " << program->String() << std::endl;
...
// bool testBooleanObject(const std::shared_ptr<Object>& obj, bool expected) {
//     auto result = std::dynamic_pointer_cast<YOXS_OBJECT::Boolean>(obj);
//     if (!result) {
//         auto& obj_ref = *obj;
//         std::cerr << "Object is not Boolean. Got=" << typeid(obj_ref).name() << std::endl;
//         return false;
//     }
//     if (result->Value != expected) {
//         std::cerr << "Object has wrong value. Got=" << result->Value << ", Want=" << expected << std::endl;
...
// bool testNullObject(const std::shared_ptr<Object> obj) {
//     // Assuming NULL is represented by a nullptr or a specific NullObject in your implementation
//     auto& obj_ref = *obj;
//     if (obj != nullptr && typeid(obj_ref) != typeid(NULL_OBJ)) {
//         std::cerr << "Object is not NULL. Got=" << typeid(obj_ref).name() << std::endl;
//         return false;
//     }
//     return true;
// }
